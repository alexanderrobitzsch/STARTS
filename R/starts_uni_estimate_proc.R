        ...
        est_var_trait,    est_var_ar, est_var_state, var_meas_error )
{
    ...
    par_names <- c( paste0('mu', 1L:W), c('var_trait', 'var_ar', 'var_state', 'a') )
    ...
    #----- output
    res <- list(W=W, pars_inits=pars_inits, time_index=time_index, covmat=covmat,
                    M=M, data0=data0, par_names=par_names, nobs=nobs, sd0=sd0,
                    pars_est=pars_est, df_sem=df_sem, par_names_est=par_names_est,
                    some_missings=some_missings, suff_stat=suff_stat,
